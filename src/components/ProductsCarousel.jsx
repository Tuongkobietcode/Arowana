...
const ProductCarousel = () => {
  ...
  const tabs = ["POPULER", "NEW ARRIVALS", "BEST SELLING"];

  // Mỗi tab đều có 8 sản phẩm
  const tabProducts = {
    POPULER: products,
    "NEW ARRIVALS": [...products].reverse(), // đảo ngược
    "BEST SELLING": [...products].sort((a, b) => a.price - b.price), // sort theo giá
  };
  ...
  }, [prevEl, nextEl, activeTab]);

  return (
    <section className="py-25 bg-white">
      <div className="max-w-7xl mx-auto px-6 text-center">
        {/* Tiêu đề */}
        <h2 className="text-[45px] font-semibold text-black">Our Products</h2>
        <p className="text-xl font-serif text-black mt-2">
          There are many variations of collections.
        </p>
        ...
        <div className="flex justify-center gap-2 mt-15">
          {tabs.map((tab) => (
            <button
              ...
              className={`px-8 py-5 font-bold transition-colors duration-300 ${
                activeTab === tab
                  ? "bg-[#071c1f] text-white"
                  : "bg-[#f2f6f7] text-black hover:bg-[#f2f6f7]"
              }`}
            >
              {tab}
            </button>
